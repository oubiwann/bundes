...
(defn changed-map
  ...
  (let [old                 (get old k {:status :stop})
    ...
    (match [(:type old)
           ...
           [_ _ _ _       :stop :stop]  []
           [:daemon _ _ _ :start :stop] [{:action :stop
                                          :unit   old}]
           [:batch  _ _ _ :start :stop] [{:action :sched-del
                                          :unit   old
                                          :sched  (:schedule old)}]

           [_ :daemon _ _ :stop :start] [{:action :start
                                          :unit   new}]
           [_ :batch  _ _ :stop :start] [{:action :sched-add
                                          :unit   new
                                          :sched  (:schedule new)}]
           ...
           [:batch :daemon _ _ _ _]     [{:action :sched-del
                                          :unit   old
                                          :sched  (:schedule old)}
                                         {:action :start
                                          :unit   new}]
           ...
           ;; Unit switches from batch to daemon
           [:daemon :batch _ _ _ _]     [{:action :stop
                                          :unit   new}
                                         {:action :sched-add
                                          :unit   new
                                          :sched  (:schedule new)}]
           ...
           [_ :daemon false _ _ _]      [{:action :stop
                                          :unit   old}
                                         {:action :start
                                          :unit   new}]

           ;; Unit schedules changed
           [_ :batch _ false _ _]       [{:action :sched-del
                                          :unit   old
                                          :sched  (:schedule old)}
                                         {:action :sched-add
                                          :unit   new
